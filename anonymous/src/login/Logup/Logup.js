...
function Logup() {
	...
	const handleValidation = () => {
		const { password, repass, userName, age, title, favorite, description } =
			values;
		if (userName.length < 3) {
			toast.error('Tên người dùng quá ngắn', toastOptions);
			return false;
			...
			toast.error('Tuổi của bạn phải lớn hơn 18', toastOptions);
			return false;
	...
	const handleSubmit = async (event) => {
		...
		if (handleValidation()) {
			...
			if (user.data.status === false || post.data.status === false) {
				toast.error(user.data.msg, toastOptions);
			} else {
	...
	return (
		<>
			<div className="log_container">
				<Link className="log_up" to="/login">
